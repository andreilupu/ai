...
interface BlockTypeWithAttributes extends ContentBlock {
	attributes?: Record<
		string,
		{ role?: string; __experimentalRole?: string } | undefined
	>;
	supports?: { contentRole?: unknown };
}

/**
 * Whether a block can carry guidelines.
 *
 * A block qualifies when it opts in with `supports.contentRole` or when at
 * least one of its attributes has the `content` role. This mirrors the
 * `isContentBlock` helper in `@wordpress/blocks`, which is a private API and so
 * cannot be imported from a plugin.
 *
 * @param block Block type from the registry.
 */
function isContentBlock( block: BlockTypeWithAttributes ): boolean {
	if ( block.supports?.contentRole ) {
		return true;
	}
...
export function useContentBlocks(): ContentBlock[] {
	return useSelect(
		( select ) =>
			select( blocksStore )
				.getBlockTypes()
				.filter( ( block: BlockTypeWithAttributes ) =>
...
/**
 * The fields the page reads off a `wp_knowledge` record.
 */
...
export function useGuidelineData(): GuidelineData {
	...
	const slugs = useMemo( () => {
		...
		const hasBlocksScope = scopes.some(
			( scope ) => scope.slug === BLOCKS_SCOPE
		);
		const list = [
			// The Blocks scope has no single row; its per-block rows are added below.
			...scopes
				.filter( ( scope ) => scope.slug !== BLOCKS_SCOPE )
				.map( ( scope ) => scopeSlug( scope.slug ) ),
			...( hasBlocksScope
				? contentBlocks.map( ( block ) => blockSlug( block.name ) )
				: [] ),
		];
	...
	const bySlug = useMemo( () => {
		...
		for ( const row of ( rowRecords ?? [] ) as KnowledgeRecord[] ) {
			map[ row.slug ] = {
				id: row.id,
