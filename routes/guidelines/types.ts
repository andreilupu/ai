...
export interface GuidelineQuery {
	...
	status: string[];
	per_page: number;
	// core-data takes queries as a plain record, so the shape has to stay
	// assignable to one.
	[ key: string ]: unknown;
}
...
/**
 * The on-disk import/export JSON shape. Flat scopes carry `{ guidelines }`;
 * `blocks` is a map of block name to `{ guidelines }`.
 */
